...
def validate_message():
    ...
    if(email_text == "") or (sub_text == "") or (msg_text == ""):
        ...
        EMAIL_REBEX = re.compile(r"[^@\s]+@[^@\s]+\.[a-zA-Z0-9]+$")
        if not EMAIL_REBEX.match(email_text):
            label9.grid()
...
f1 = Frame(root, width=100, height=800)
f1.pack(side=TOP)
...
label4 = Label(f2, width=20, text="Log in Success", font=("Calibri 12 bold"), bg="cyan", fg="red")
label4.grid(row=0, column=0, pady=5, columnspan=2)
...
label5.grid(row=0, pady=10, columnspan=3)
